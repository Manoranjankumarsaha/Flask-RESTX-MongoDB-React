...
ns2 = api.namespace('api2', description='My API-2')
...
class GetAndPost(Resource):
    ...
    def post(self):
        data = api.payload
        course = courses(courseID=data['courseID'], title=data['title'],
                         description=data['description'], credits=data['credits'], term=data['term'])
        course.save()
...
class GetAndPost(Resource):
    def get(self):
        return jsonify({'data': [{v: v} for v in range(1, 11)]})
